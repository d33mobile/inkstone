...
impl RelearnState {
    ...
    fn answer_again(self, ctx: &StateContext) -> CardState {
        let scheduled_days = self.review.failing_review_interval(ctx);
    ...
    fn answer_easy(self, _ctx: &StateContext) -> ReviewState {
        let scheduled_days = self.review.scheduled_days + 1;
        ReviewState {
            scheduled_days,
            elapsed_days: 0,
            ..self.review
